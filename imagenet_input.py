...
class ImageNetTFExampleInput(object):
  ...
  def dataset_parser(self, value):
    ...
    keys_to_features = {
        ...
        'image/label': tf.FixedLenFeature([5], tf.float32),#update
    }
    ...
    # Subtract one so that labels are in [0, 1000). #update
    label = tf.cast(
        tf.reshape(parsed['image/label'], shape=[5]), dtype=tf.float32)
    return image, label
...
class ImageNetInput(ImageNetTFExampleInput):
  ...
  def make_source_dataset(self, index, num_hosts):
    ...
    file_pattern = os.path.join(
        self.data_dir, 'train-*' if self.is_training else 'valid-*')
