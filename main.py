...
flags.DEFINE_integer(
    'num_train_images', default=200, help='Size of training data set.')

flags.DEFINE_integer(
    'num_eval_images', default=200, help='Size of evaluation data set.')
...
def model_fn(features, labels, mode, params):
  ...
  #update
  # features = tf.image.resize_images(features, (64,64))
  ...
  # else:
  endpoints = build_model()

  x_global_pool = tf.reduce_mean(endpoints, axis=[1,2])
  print('###################global_pool', x_global_pool)
  logits = tf.contrib.layers.fully_connected(x_global_pool, 5, activation_fn=None)
  print('###################global_pool', logits)
  time.sleep(10)

  if mode == tf.estimator.ModeKeys.PREDICT:
    predictions = {
        'classes': tf.argmax(logits, axis=1),
        'probabilities': tf.math.sigmoid(logits, name='softmax_tensor')
    }
  ...
  # one_hot_labels = tf.one_hot(labels, FLAGS.num_label_classes) #update
...
def main(unused_argv):
  ...
  est = tf.contrib.tpu.TPUEstimator(
      ...
      params=params,
      warm_start_from='./weights_efficientnet-b0/model.ckpt'
      ) #update
